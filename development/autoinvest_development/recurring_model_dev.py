...
NOTEBOOK_NAME = "recurring_model_dev"
...
# MAGIC 
# MAGIC Using pipeline and best practices, develop a model with pipeline capabilities. Model will use pysparks GBTClassifier to predict probability a sample taking the recurring_action
# COMMAND ----------
...
positive_class = spark.read.table("multimodel_forecaster.recurring_users_positive_class")
negative_class = spark.read.table("multimodel_forecaster.recurring_users_negative_class")
data = positive_class.union(negative_class)

# COMMAND ----------

cols_to_drop = ['uuid','recurring_action_binary', 'snapshot_date', 'target']
features = [i for i in data.columns if i not in cols_to_drop]
catFeatures = [i for i in features if '00' not in i]
...
for c in acxiom_features:
  data = data.withColumn(c,
                        F.col(c).cast('float'))
data = data.withColumnRenamed("recurring_action_binary", 'target')
...
with mlflow.start_run(run_name = model_training_config['run_name']) as run:
  #log parameters
  mlflow.log_params({'data': 'multimodel_forecaster.autoinvest_users',
                     'positive_class_data': 'multimodel_forecaster.recurring_users_positive_class',
                     'downsample_negative_class_data': 'multimodel_forecaster.recurring_users_negative_class',
                     'target': 'recurring_action_binary'})
  ...
  mlflow.spark.log_model(bestModel, 'log-test')
