...
NOTEBOOK_NAME = "longterm_model_dev"
...
# MAGIC 
# MAGIC Using pipeline and best practices, develop a model with pipeline capabilities. Model will use pysparks GBTClassifier to predict probability of a sample taking a longterm action
...
positive_class = spark.read.table("multimodel_forecaster.longterm_users_positive_class")
negative_class = spark.read.table("multimodel_forecaster.longterm_users_negative_class")
data = positive_class.union(negative_class)

# COMMAND ----------

cols_to_drop = ['uuid','longterm_binary', 'snapshot_date', 'target']
...
for c in acxiom_features:
  data = data.withColumn(c,
                        F.col(c).cast('float'))
data = data.withColumnRenamed("longterm_binary", 'target')
...
with mlflow.start_run(run_name = model_training_config['run_name']) as run:
  #log parameters
  mlflow.log_params({'data': 'multimodel_forecaster.later_users',
                     'positive_class_data': 'multimodel_forecaster.longterm_users_positive_class',
                     'downsample_negative_class_data': 'multimodel_forecaster.longterm_users_negative_class',
                     'target': 'longterm_binary'})
  ...
  mlflow.spark.log_model(bestModel, 'best-model-long-term')
