...
NOTEBOOK_NAME = "next_positive_target_eda_and_generation"
...
features = feature_config['next_positive']
...
users = users.select("id", *features, 'next_positive_binary')
...
users.groupby('next_positive_binary').agg(F.count(F.lit(1))).display()
...
positive_class = users.filter(F.col("next_positive_binary") == F.lit(1))
negative_class_all = users.filter(F.col("next_positive_binary") == F.lit(0))
...
get_acxiom_codes(users) \
     .withColumn('snapshot_date',
      ...
      .write.mode("overwrite").option("overwriteSchema", True).saveAsTable("multimodel_forecaster.next_positive_users_full_population")

get_acxiom_codes(positive_class).withColumn('snapshot_date',
                          F.lit(F.current_date())).write.mode("overwrite").option("overwriteSchema", True).saveAsTable("multimodel_forecaster.next_positive_users_positive_class")

get_acxiom_codes(negative_class).withColumn('snapshot_date',
                          F.lit(F.current_date())).write.mode("overwrite").option("overwriteSchema", True).saveAsTable("multimodel_forecaster.next_positive_users_negative_class")

get_acxiom_codes(negative_class_all).withColumn('snapshot_date',
                          F.lit(F.current_date())).write.mode("overwrite").option("overwriteSchema", True).saveAsTable("multimodel_forecaster.next_positive_users_negative_class_all")
