...
NOTEBOOK_NAME = "small_action_target_eda_and_generation"
...
features = feature_config['small_action']
...
users = users.select("id", *features, 'small_action_binary')
...
users.groupby('small_action_binary').agg(F.count(F.lit(1))).display()
...
positive_class = users.filter(F.col("small_action_binary") == F.lit(1))
negative_class_all = users.filter(F.col("small_action_binary") == F.lit(0))
...
get_acxiom_codes(users) \
     .withColumn('snapshot_date',
      ...
      .write.mode("overwrite").option("overwriteSchema", True).saveAsTable("multimodel_forecaster.small_action_users_full_population")

get_acxiom_codes(positive_class).withColumn('snapshot_date',
                          F.lit(F.current_date())).write.mode("overwrite").option("overwriteSchema", True).saveAsTable("multimodel_forecaster.small_action_users_positive_class")

get_acxiom_codes(negative_class).withColumn('snapshot_date',
                          F.lit(F.current_date())).write.mode("overwrite").option("overwriteSchema", True).saveAsTable("multimodel_forecaster.small_action_users_negative_class")

get_acxiom_codes(negative_class_all).withColumn('snapshot_date',
                          F.lit(F.current_date())).write.mode("overwrite").option("overwriteSchema", True).saveAsTable("multimodel_forecaster.small_action_users_negative_class_all")
