...
NOTEBOOK_NAME = "small_action_model_dev"
...
# Magic Model will use pysparks GBTClsasifier Forecast samples propensity to take the small_action action
...
positive_class = spark.read.table("multimodel_forecaster.small_action_users_positive_class")
negative_class = spark.read.table("multimodel_forecaster.small_action_users_negative_class")
data = positive_class.union(negative_class)

# COMMAND ----------

cols_to_drop = ['uuid','small_action_binary', 'snapshot_date', 'target']
...
for c in acxiom_features:
  data = data.withColumn(c,
                        F.col(c).cast('float'))
data = data.withColumnRenamed("small_action_binary", 'target')
...
with mlflow.start_run(run_name = model_training_config['run_name']) as run:
  #log parameters
  mlflow.log_params({'data': 'multimodel_forecaster.small_action_users',
                     'positive_class_data': '"multimodel_forecaster.small_action_users_positive_class',
                     'downsample_negative_class_data': '"multimodel_forecaster.small_action_users_negative_class',
                     'target': 'small_action_binary'})
  ...
  mlflow.spark.log_model(bestModel, 'best-model-small_action')
